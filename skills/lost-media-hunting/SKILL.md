...
| **Identify** | `identify_fingerprint` / `audio_match` (lostwave), `identify_transcribe`, `identify_ocr`, `identify_probe`, `identify_frames` (video→keyframes), `image_reverse`, `flash_review`, `executable_review` |
...
- Obscure file sites carry real malware/scam/shock-content risk. Downloads land as inert artifacts. Use `executable_review` in its default `static` mode first. Dynamic review requires explicit `mode:'sandbox', allowExecution:true`, always disables networking, and runs only in DOSBox, gVisor, or a resettable Hyper-V guest; if that worker is unavailable, stop rather than running on the host. **Never enter credit-card details into an untrusted site** — legitimate lost-media tools are overwhelmingly free; payment demands are usually scams.
