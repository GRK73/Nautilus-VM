...
### 7.6 `audio_match` — 후보군 대량 비교 (lostwave 종결자) ⭐ *구현됨*
...
| 단계 | 방식 | OSS | 잡아내는 것 | 출력 |
|---|---|---|---|---|
| **A. 지문 매칭** (정밀) | landmark peak-pair 해시 + 오프셋 투표 | **NumPy + SciPy + librosa** (Dejavu 방식의 무DB 구현) | *동일 녹음*이 후보 안에 들어있나 — 압축·노이즈·앞뒤 잘림에 강함, **시간 오프셋**까지 | `{candidateId, offsetSec, alignedHashes, score}` |
| **B. 특징 유사도** (퍼지) | 구간 음악 특징 + subsequence DTW | **librosa** (chroma CENS + MFCC) | *다른 녹음*(커버·리마스터·재생속도) — 지문이 안 붙는 경우의 graded 유사도 | `{candidateId, similarity 0..1, offsetSec}` |

- **A가 히트하면 B는 생략**(정밀 일치 = 확정). A에서 안 걸린 후보만 B로 넘겨 0..1 유사도로 랭킹. 두 점수는 **방식 라벨을 달아 분리 제시**(fingerprint 히트는 항상 fuzzy 위에 랭크) — 거짓 동일축 비교 금지.
- **Landmark 흐름**: 후보별 peak-pair 지문을 아티팩트 SHA-256 키로 케이스 캐시에 적재 → 레퍼런스와 공유 해시의 시간차를 투표 → "이 클립이 들어있는 후보 + 오프셋 + 정렬 해시 수" 반환. 외부 SQL이 필요한 Dejavu 대신 같은 핵심 알고리즘을 무DB 배치 처리로 구현했다.
- **특징 흐름**: librosa의 chroma CENS와 정규화 MFCC 시퀀스를 각각 subsequence DTW로 비교하고 합성한다. 현재 slim Python 이미지에 배포 가능한 Essentia wheel이 없어 전역 descriptor는 제외했으며, 구간 위치 검출에는 subsequence 특징이 더 직접적이다.
...
audio_match { referenceId, candidateIds[], mode?: 'auto'|'fingerprint'|'features', topK? }
  → AudioMatcher (packages/identify)         # 아티팩트 id → blob 경로 해석
    → docker run nautilus-audio-match          # network=none, artifacts read-only
      → python tools/audio-match/match.py      # landmark hashing / librosa subsequence DTW
...
- **Graceful degradation**: 이미지/Docker 부재 시 빌드 명령을 포함한 구조화 오류를 반환한다. 컨테이너는 `--network none`, 아티팩트는 read-only, 파생 캐시만 read-write로 실행한다.
